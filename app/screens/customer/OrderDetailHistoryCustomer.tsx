...
const OrderDetailHistoryCustomer = () => {
    ...
    React.useEffect(() => {
        const fetchChatHistory = async () => {
            ...
            try {
                ...
                console.log(`[OrderDetailHistory] Fetching chat from ${chatTable} for order ${orderId}`);
                ...
                if (error) {
                    console.error('[OrderDetailHistory] Error fetching chat:', error);
                } else if (data) {
                    console.log(`[OrderDetailHistory] Found ${data.length} chat messages`);
                    const formattedMessages: ChatMessage[] = data.map((msg: any, index: number) => ({
                        id: msg.id || `msg-${index}`,
                        sender: index % 2 === 0 ? 'driver' : 'customer', // Alternate for now - TODO: add sender field to chat tables
                ...
                console.error('[OrderDetailHistory] Exception fetching chat:', err);
            } finally {
                setIsLoadingChat(false);
            }
        };
                    ...
                    <View style={styles.card}>
                        <View style={styles.cardHeader}>
                            ...
                            <View style={styles.cardHeaderText}>
                                ...
                                <Text style={styles.cardValue}>{driverName || 'Driver'}</Text>
                            </View>
                        </View>
                    </View>
                            ...
                            <View style={styles.chatContainer}>
                                {chatMessages.map((msg, idx) => (
                                    <View key={msg.id || idx} style={[
                                        styles.chatBubble,
                                        msg.sender === 'customer' ? styles.chatCustomer : styles.chatDriver
                                    ]}>
                                        {msg.imageUrl && (
                                        ...
                                        {msg.text ? (
                                            <Text style={[
                                                styles.chatText,
                                                msg.sender === 'customer' ? styles.chatTextWhite : styles.chatTextDark
                                            ]}>{msg.text}</Text>
                                        ) : null}
                                        <Text style={[
                                            styles.chatTime,
                                            msg.sender === 'customer' ? styles.chatTimeWhite : styles.chatTimeDark
                                        ]}>{msg.time}</Text>
                                    </View>
                                ))}
                            </View>
...
export default OrderDetailHistoryCustomer;
