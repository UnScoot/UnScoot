...
import { updateOrderStatus, subscribeToOrderStatus } from '../../../../src/database/chatScootSend';
import { supabase } from '../../../../src/database/supabase';

const SendValidasi: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const handleStatusChange = (newStatus: string) => {
            ...
            if (newStatus === 'completed') {
                isNavigating = true;
                console.log('[SendValidasi] Order completed! Redirecting to rating...');
                ...
                router.replace({
                    pathname: '/screens/customer/ScootSendCustomer/SendRating',
                    params: { orderId, userId }
                } as any);
            }
        };

        console.log('[SendValidasi] Setting up order status subscription for order:', orderId);

        // Realtime subscription
        const subscription = subscribeToOrderStatus(orderId, (payload: any) => {
            console.log('[SendValidasi] Order status changed via realtime:', payload.new?.status);
        ...
        const checkOrderStatus = async () => {
            ...
            try {
                ...
                if (!error && data) {
                    console.log('[SendValidasi] Polling check - current status:', data.status);
                    handleStatusChange(data.status);
                }
            } catch (err) {
                console.error('[SendValidasi] Polling error:', err);
            }
        };
        ...
        <>
            ...
            <SafeAreaView style={styles.container}>
                <View style={styles.card}>
                    ...
                    <Text style={styles.title}>Apakah barang kamu sudah sampai?</Text>

                    <Text style={styles.subtitle}>Klik tombol di bawah kalau barangmu{"\n"}udah kamu terima ya ☺️</Text>
