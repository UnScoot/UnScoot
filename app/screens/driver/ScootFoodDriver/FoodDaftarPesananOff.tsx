...
const FoodDaftarPesananOff = () => {
	const offImage = require('../../../../assets/images/Off.png');
	const router = useRouter();
	const params = useLocalSearchParams();
	const { userId, nama, nim, email, jenisMotor, plat } = params;
	...
	React.useEffect(() => {
		const resolveUserId = async () => {
			...
			if (finalUserId) {
				console.log('[Daftar_Pesanan_ScootFood_Off] Using userId from params:', finalUserId);
			...
			try {
				...
				if (userSession) {
					...
					if (finalUserId) {
						console.log('[Daftar_Pesanan_ScootFood_Off] Using userId from AsyncStorage:', finalUserId);
						setResolvedUserId(finalUserId);
						return;
					}
				}
			} catch (e) {
				console.warn('[Daftar_Pesanan_ScootFood_Off] Error reading AsyncStorage:', e);
			}

			console.error('[Daftar_Pesanan_ScootFood_Off] ❌ Could not resolve userId!');
		};
	...
	const handleToggle = () => {
		router.push({
			pathname: '/screens/driver/ScootFoodDriver/FoodDaftarPesanan',
...
export default FoodDaftarPesananOff;