 ...
 */
export async function registerUser({ nim, email, password, nama, jenisMotor, plat, role }) {
  ...
  console.log("[registerUser] Input data:", { nim, email, nama, role, jenisMotor, plat });
  ...
  console.log("[registerUser] User metadata to save:", JSON.stringify(userMetadata, null, 2));
  ...
  console.log("[registerUser] User created with metadata:", JSON.stringify(data?.user?.user_metadata, null, 2));
  ...
  if (data?.user) {
    ...
    if (insertError) {
      console.warn(`[registerUser] Warning: Could not insert to ${table} immediately:`, insertError.message);
      console.log("[registerUser] Data will be inserted when user logs in after email verification");
